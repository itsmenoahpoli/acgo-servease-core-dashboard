...
export default function Tenants() {
  ...
  const createMutation = useMutation({
    mutationFn: (name: string) => tenantService.createTenant({ name }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      tenantService.updateTenant(id, { enabled }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tenants"] });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <Spinner label="Loading service providers..." />
      </div>
    );
  }
    ...
    return (
      <div className="p-6">
        <div className="text-center py-8">
          <p className="text-red-600">Error loading service providers. Please try again.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6">
      ...
      {showCreateForm && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <Formik
            initialValues={{ name: "" }}
            validate={createFormikValidator(tenantSchema)}
